...
Your `GuestBookingRestService` must provide a single method which accepts a POST request containing a `GuestBooking` parameter. This method should then use the `CustomerService` and `BookingService` classes to persist the appropriate fields of the `GuestBooking` object.
