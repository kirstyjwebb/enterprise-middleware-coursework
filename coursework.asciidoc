...
TIP: The automatic deletion of related entities should be handled with JPA link:http://docs.oracle.com/javaee/6/tutorial/doc/bnbqa.html[Entity relationship @Annotations] (e.g. `@ManyToOne`, `@ManyToMany` etc...), which are provided by link:https://docs.jboss.org/hibernate/stable/annotations/reference/en/html_single/#entity-mapping-association[Hibernate].
