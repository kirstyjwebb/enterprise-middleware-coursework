...
In Part Three you will create a new, more complex, service. It will make requests to your Part One service, along with those of two colleagues. This service should also be accompanied by more tests and JSON endpoints. These endpoints, and their responses, should match those expected by an [AngularJS](https://angularjs.org/) client application, which will be provided. If done correctly, you will have a functioning single-page web application (SPA) after this step.
...
You must document your endpoints using Swagger [@Annotations](https://github.com/swagger-api/swagger-core/wiki/Annotations-1.5.X), including:
...
TIP: The automatic deletion of related entities should be handled with Hibernate [relationship @Annotations](https://docs.jboss.org/hibernate/stable/annotations/reference/en/html_single/#entity-mapping-association) (e.g. `@ManyToOne`, `@ManyToMany` etc...).
...
* `src/foo/bar/Booking.js`
* `src/foo/bar/Customer.js`
* `src/foo/bar/Flight.js`
* `src/foo/bar/Hotel.js`
* `src/foo/bar/Taxi.js`
